...
VALUES
    ...
    ('透明人', 'opcatiy0@hexschooltest.io', 'USER') -- 1-2 修改：用 Email 找到 李燕容、肌肉棒子、Q太郎，如果他的 Role 為 USER 將他的 Role 改為 COACH
UPDATE "USER"
SET
    role = 'COACH'
WHERE
...
VALUES
    ('7 堂組合包方案', '1400', '7'),
    ('14 堂組合包方案', '2520', '14'),
    ('21 堂組合包方案', '4800', '21') -- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    -- 1. `王小明` 購買 `14 堂組合包方案`
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`
INSERT INTO
...
SELECT
    "USER".id AS user_id,
    (
        "CREDIT_PURCHASE".purchased_credits - COALESCE(used_credits.total, 0)
    ) AS remaining_credit
FROM
    "USER"
    ...
    LEFT JOIN (
        ...
        GROUP BY
            "COURSE_BOOKING".user_id
    ) AS used_credits ON "USER".id = used_credits.user_id
WHERE
